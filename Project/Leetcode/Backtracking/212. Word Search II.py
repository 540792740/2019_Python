...
class Solution:
    ...
    def find(self, board, i, j, trie, path):
        m = len(board)
        n = len(board[0])
        if '#' in trie and path not in self.res: self.res.append(path)
        if not self.used[i][j] and board[i][j] in trie:
            ...
            for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0),(0, 0)]:
                ...
                y = j + dy
                if -1 < x < m and -1 < y < n:
                    self.find(board, x, y, trie[board[i][j]], path + board[i][j])
            self.used[i][j] = False


    # TLE
    def findWords1(self, boards, words):
        def helper(word_index, x, y, word, board):
            if board[x][y] != word[word_index]:
                return False
            elif word_index == ls - 1:
                return True
            else:
                word_index += 1
                tempChar = board[x][y]
                ...
                for d in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                    ...
                    if -1 < x_next < m and -1 < y_next < n and board[x_next][y_next]:
                        if helper(word_index, x_next, y_next, word, board): return True
                board[x][y] = tempChar
                return False
...
if __name__ == '__main__':
    S = Solution()
    # test = S.findWords([["A","B","C","E"],["S","F","C","S"],["A","D","E","E"]], "ABCCED")
    # test = S.findWords([["o","a","a","n"],["e","t","a","e"],["i","h","k","r"],["i","f","l","v"]], ["oath","pea","eat","rain"])
    test = S.findWords([["a"]],["a"])
